...
\renewcommand{\algorithmicensure}{\textbf{Output:}}
%Input
\REQUIRE $ \newline initial\_estimated\_state = P(x_{0} | e_{0})=bel(x_{0}), \text{ The initial estimated state}
\newline max\_no\_targets, \text{ the maximum number of targets to be localised.}
\newline find\_next\_target, \text{}$
%Output
\ENSURE  $\newline P(X_{t} | e_{1:t}) = bel(X_{t})$
